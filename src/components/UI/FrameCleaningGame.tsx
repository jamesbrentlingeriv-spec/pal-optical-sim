...
export const FrameCleaningGame: React.FC<FrameCleaningGameProps> = ({ onClose, onComplete, brand }) => {
  const { width } = useWindowSize();
  const isMobile = width < 768;
  const scale = isMobile ? Math.min(1, width / 700) : 1;

  const frameImage = getFrameImage(brand || null);
  const [dustPoints, setDustPoints] = useState<{ x: number; y: number; cleaned: boolean }[]>([]);
  ...
  const handleMouseMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    setDustPoints(prev => {
      ...
      const next = prev.map(p => {
        if (!p.cleaned && Math.abs(p.x - relX) < 8 && Math.abs(p.y - relY) < 8) {
          changed = true;
    ...
    >
      ...
      >
        ...
        <div className="text-center mb-6">
          <h2 className="text-4xl font-black italic uppercase tracking-tighter mb-2">Clean the Frames</h2>
          {brand && <p className="text-gray-500 font-bold text-sm uppercase tracking-wider">{brand} Display</p>}
          <p className="text-gray-600 font-bold">Wipe away the dust with your microfiber cloth!</p>
        </div>
        ...
        >
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(180deg, #8B4513 0%, #6B3410 30%, #5C2E0E 60%, #4A250B 100%)',
            }}
          />
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ paddingBottom: '12%' }}>
            {frameImage ? (
              ...
              <svg viewBox="0 0 200 100" className="w-1/2 h-auto text-gray-800 fill-none stroke-gray-700 stroke-[3px] drop-shadow-lg">
                <path d="M20,50 Q20,20 60,20 Q100,20 100,50 M140,20 Q180,20 180,50 Q180,80 140,80 Q100,80 100,50 Q100,80 60,80 Q20,80 20,50" strokeLinecap="round" />
                <path d="M60,20 Q100,15 140,20" />
                <ellipse cx="50" cy="50" rx="20" ry="20" fill="rgba(255,255,255,0.15)" />
                <ellipse cx="150" cy="50" rx="20" ry="20" fill="rgba(255,255,255,0.15)" />
              </svg>
            )}
          </div>
        ...
        <div className="mt-8">
          <div className="flex justify-between items-end mb-2">
            <span className="font-black text-sm uppercase">Cleaning Progress</span>
