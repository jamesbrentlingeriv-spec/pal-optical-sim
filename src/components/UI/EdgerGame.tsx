...
const LENS_SHAPES = [
  {
    name: "ROUND",
    label: "ROUND",
    getPath: (cx: number, cy: number, w: number, h: number) => {
    ...
    switch (path.type) {
      ...
      case "rect": {
        ...
        if (d < path.w) {
          px = cx - hw + d;
          py = cy - hh;
        } else if (d < path.w + path.h) {
          px = cx + hw;
          py = cy - hh + (d - path.w);
        } else if (d < 2 * path.w + path.h) {
          px = cx + hw - (d - path.w - path.h);
          py = cy + hh;
        } else {
          px = cx - hw;
          py = cy + hh - (d - 2 * path.w - path.h);
        }
        break;
      }
      case "aviator": {
      ...
      case "wayfarer": {
        ...
        const hw = path.w / 2,
          hh = path.h / 2;
        const topInset = 10;
        const topFlatStart = 0.15,
          topFlatEnd = 0.35;
        const peri =
          hw * 2 - topInset * 2 + path.h * 2 + topInset * 2 * Math.SQRT2;
        if (t < 0.25) {
          px = cx - hw + t * 4 * (hw - topInset);
          py = cy - hh;
        } else if (t < 0.5) {
          px = cx + hw;
          py = cy - hh + (t - 0.25) * 4 * path.h;
        } else if (t < 0.75) {
          px = cx + hw - (t - 0.5) * 4 * path.w;
          py = cy + hh;
        } else {
          px = cx - hw;
          py = cy + hh - (t - 0.75) * 4 * path.h;
        }
        break;
      }
      case "cateye": {
...
export const EdgerGame: React.FC<EdgerGameProps> = ({ onClose }) => {
  const [phase, setPhase] = useState<"IDLE" | "LOADING" | "CUTTING" | "DONE">(
    "IDLE",
  );
  ...
  const selectedLensPath = LENS_SHAPES[selectedShape].getPath(
    200,
    155,
    lensSize,
    lensSize *
      (selectedShape === 1 ? 0.65 : [0, 1].includes(selectedShape) ? 1 : 0.78),
  );
  ...
  const draw = useCallback(() => {
    ...
    if (lensLoaded || phase === "CUTTING" || phase === "DONE") {
      ...
      if (phase === "IDLE" || phase === "LOADING") {
        ...
        if (cutEndIdx > 0) {
          ...
          const firstAngleToCenter = Math.atan2(
            firstPt.y - lcy,
            firstPt.x - lcx,
          );
        ...
        const cutPointCount = Math.floor(
          (cutProgress / 100) * pathPoints.length,
        );
    ...
    // Active light
    ctx.fillStyle =
      phase === "CUTTING" ? COLORS.indicatorOn : COLORS.indicatorOff;
    ...
    <div className="fixed inset-0 z-300 bg-black/95 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          ...
          <div
            className="relative border-4 border-[#2a3a3a] rounded overflow-hidden mb-3"
            style={{
              width: CANVAS_W,
              height: CANVAS_H,
          ...
          <div className="flex gap-3 items-end">
            ...
            <div className="flex-1">
              <div className="text-[7px] text-[#6a7a7a] font-bold uppercase tracking-wider mb-1">
                Lens Shape
              </div>
            ...
            <div className="flex flex-col gap-1">
              <div className="text-[7px] text-[#6a7a7a] font-bold uppercase tracking-wider text-center">
                Size
              </div>
              <button
                onClick={() => {
                  if (phase === "IDLE") {
                    setLensSize((prev) => Math.min(60, prev + 4));
                  }
                }}
              ...
              <button
                onClick={() => {
                  if (phase === "IDLE") {
                    setLensSize((prev) => Math.max(32, prev - 4));
                  }
                }}
                className="h-7 px-2 text-[7px] font-bold bg-[#3a4a3a] border-2 border-[#2a3a2a] text-[#8a9a8a] hover:bg-[#4a5a4a] rounded-sm"
                disabled={phase !== "IDLE"}
            ...
            <div className="flex flex-col gap-1 min-w-20">
              ...
              {!lensLoaded && phase === "IDLE" ? (
                ...
                <div className="h-10 bg-[#2a3a2a] border-2 border-[#1a2a1a] flex items-center justify-center rounded-sm">
                  <span className="text-[#88cc88] text-[8px] font-bold animate-pulse">
                    LOADING...
                  </span>
                </div>
              ) : phase === "CUTTING" ? (
                <div className="h-10 bg-[#2a2a2a] border-2 border-[#1a1a1a] flex items-center justify-center rounded-sm">
                  <span className="text-[#ff4444] text-[8px] font-bold animate-pulse">
                    CUTTING...
                  </span>
                </div>
              ) : phase === "DONE" ? (
            ...
            <div className="mt-2 h-3 border-2 border-[#2a3a2a] rounded-sm overflow-hidden bg-[#0a0a0a]">
              <motion.div
                ...
                style={{
                  width: `${cutProgress}%`,
                  background:
                    "linear-gradient(90deg, #ff4444, #ff6644, #ff8844)",
          ...
          <div className="mt-2 text-center">
            ...
            >
              ...
              {phase === "IDLE" &&
                lensLoaded &&
                `LENS LOADED - ${LENS_SHAPES[selectedShape].label} ${lensSize}mm - READY TO CUT`}
