...
export default function Phone({ onClose }: PhoneProps) {
  ...
  const [currentScenario] = useState(() => {
    const s =
      PHONE_SCENARIOS[Math.floor(Math.random() * PHONE_SCENARIOS.length)];
    return {
      question: s.q,
  ...
  const handleOptionClick = (index: number, correct: boolean) => {
    ...
    setFeedback(
      correct
        ? "Great answer! The patient is satisfied."
        : "That's not quite right. Try to be more helpful.",
    );
      ...
      >
        ...
        <div className="bg-black p-6 flex justify-between items-center border-b-4 border-white">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-blue-400 text-[6px] font-black underline uppercase">
                Channel_01_Active
              </div>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-white hover:text-red-500 border-2 border-transparent hover:border-red-500"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="p-8 space-y-8">
          <div className="flex flex-col gap-4">
            <div className="text-[6px] text-blue-400 font-black tracking-[4px]">
              CALLER:
            </div>
          ...
          <div className="space-y-4 pt-4 border-t-2 border-white/20">
            <p className="text-[6px] font-black text-slate-500 uppercase tracking-widest px-2">
              Select Response
            </p>
