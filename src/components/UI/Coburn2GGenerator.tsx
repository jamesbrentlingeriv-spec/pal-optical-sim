...
const RollerCounter: React.FC<{ value: number; label: string }> = ({
  value,
  label,
}) => (
  <div className="flex flex-col items-center gap-0.5">
    <div className="text-[5px] font-mono text-slate-500 uppercase tracking-widest">
      {label}
    </div>
...
export const Coburn2GGenerator: React.FC<Coburn2GGeneratorProps> = ({
  onClose,
}) => {
  // ── Core game phase ──────────────────────────────────────────────────────
  type Phase =
  ...
  const [dataInput, setDataInput] = useState<Record<DataField, string>>({
    sphere: "",
    cylinder: "",
    axis: "",
  });
    ...
    (e: KeyboardEvent) => {
      ...
      if (phase === "JOB_INPUT") {
        if (/^\d$/.test(key) && jobNumber.length < 4) {
          ...
          (key === "Enter" || key === "NumpadEnter" || key === "numpadenter") &&
          jobNumber.length === 4
        ) {
      ...
      if (phase === "DATA_ENTRY") {
        ...
        if (key === "Enter" || key === "NumpadEnter" || key === "numpadenter") {
          ...
          if (currentField === "axis") {
            ...
            const next: Record<DataField, DataField> = {
              sphere: "cylinder",
              cylinder: "axis",
              axis: "axis",
            };
        ...
        const fieldLimits: Record<DataField, number> = {
          sphere: 5,
          cylinder: 5,
          axis: 3,
        };
  ...
  const performTimingHit = useCallback(() => {
    ...
    if (indicatorEdge >= zoneStart && indicatorEdge <= zoneEnd) {
      ...
      hitType =
        Math.abs(indicatorEdge - center) <= perfectRadius ? "perfect" : "ok";
    } else if (
      indicatorEdge < 0 + crashThreshold ||
      indicatorEdge > 100 - crashThreshold
    ) {
      hitType = "crash";
    } else {
      hitType = "miss";
  ...
  const calculateResult = useCallback(() => {
    ...
    const dataAccuracy =
      sphCorrect && cylCorrect && axisCorrect
        ? 40
        : (((sphCorrect ? 1 : 0) +
            (cylCorrect ? 1 : 0) +
            (axisCorrect ? 1 : 0)) /
    ...
    const timingScore =
      (perfectCount / totalHits) * 60 + (okCount / totalHits) * 35;
  ...
  const crtTitle = () => {
    switch (phase) {
      case "JOB_INPUT":
        return "ENTER JOB TICKET:";
      case "SHOW_SHAPE":
        return `JOB #${jobNumber} - SHAPE ASSIGNED`;
      case "DATA_ENTRY":
        return "KEY PRESCRIPTION DATA";
      case "LOCKING":
        return "CHUCK LOCKING...";
      case "READY_CUT":
        return "READY - PRESS START";
      case "CUTTING":
        return "CUTTING CYCLE ACTIVE";
      case "RESULT":
        return `SCORE: ${score}%`;
      case "FAILURE":
  ...
  const crtSubText = () => {
    ...
    if (phase === "DATA_ENTRY" && dataTimer !== null)
      return `TIME: ${dataTimer.toFixed(1)}s`;
    return "";
  };
  ...
  return (
    <div className="fixed inset-0 z-300 bg-black/90 flex items-center justify-center p-4 font-mono select-none">
      ...
      <motion.div
        animate={
          screenShake
            ? { x: [0, -6, 6, -4, 4, -2, 2, 0], y: [0, 4, -4, 3, -3, 2, -2, 0] }
            : {}
        }
      ...
      >
        ...
        <div
          ...
          style={{
            width: 520,
            background:
              "linear-gradient(170deg, #b0b8b8 0%, #8a9494 30%, #7a8484 60%, #6a7474 100%)",
          }}
        >
          {/* Machine Brand Strip */}
            ...
            <div className="flex flex-col gap-2" style={{ width: 280 }}>
              ...
              >
                ...
                <div className="bg-[#3a4242] px-2 py-0.5 flex items-center justify-between border-b-2 border-[#2a3232]">
                  <span className="text-[5px] text-[#889898] font-bold tracking-widest uppercase">
                    CRT
                  </span>
                  <span className="text-[5px] text-[#ff6666] font-bold">
                    ● ON
                  </span>
                </div>
                ...
                <div
                  ...
                  style={{
                    background:
                      "radial-gradient(ellipse at center, #1a1a4e 0%, #0e0e2e 60%, #08081a 100%)",
                ...
                >
                  ...
                  <div
                    ...
                    style={{
                      backgroundImage:
                        "repeating-linear-gradient(0deg, transparent, transparent 2px, rgba(0,0,40,0.3) 2px, rgba(0,0,40,0.3) 4px)",
                    }}
                  />
                  {/* CRT glow effect */}
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(ellipse at 30% 40%, rgba(100, 100, 255, 0.08) 0%, transparent 70%)",
                    }}
                  />
                  ...
                  <div className="relative h-full p-3 flex flex-col">
                    ...
                    {/* Title line */}
                    <div
                      className="text-[9px] font-bold text-[#7a7aff] mb-1.5"
                      style={{ textShadow: "0 0 6px rgba(100,100,255,0.5)" }}
                    >
                      {">"} {crtTitle()}
                      {crtBlink && (
                    ...
                    {phase === "JOB_INPUT" && (
                      <div className="flex-1 flex flex-col items-center justify-center gap-1">
                        <div
                          className="text-[20px] font-bold text-[#aaaaff]"
                          style={{
                        ...
                        >
                          {jobNumber
                            ...
                            .map((c, i) => (
                              <span
                                key={i}
                                className={c === "_" ? "opacity-30" : ""}
                              >
                                {c}
                              </span>
                            ))}
                        </div>
                        <div className="text-[6px] text-[#6666aa] mt-1">
                          ENTER 4-DIGIT JOB # THEN ENTER
                        </div>
                      </div>
                    )}
                      ...
                      <div className="flex-1 flex flex-col items-center justify-center gap-1">
                        ...
                        >
                          <path
                            ...
                            style={{
                              filter:
                                "drop-shadow(0 0 4px rgba(100,150,255,0.6))",
                            }}
                          />
                        </svg>
                        ...
                        <div className="text-[6px] text-[#556688] mt-0.5">
                          TICKET #{jobNumber}
                        </div>
                        <div className="text-[6px] text-[#7799aa] mt-0.5 animate-pulse">
                    ...
                    {phase === "DATA_ENTRY" && (
                      <div className="flex-1 flex flex-col gap-0.5">
                        ...
                        <div className="border border-[#4444aa]/30 bg-[#111133]/50 rounded px-1.5 py-0.5 mb-0.5">
                          <div className="text-[5px] text-[#6666aa] uppercase tracking-widest">
                            Work Ticket #{jobNumber}
                          </div>
                        </div>

                        {/* Fields */}
                        {(["sphere", "cylinder", "axis"] as DataField[]).map(
                          (field) => (
                            <div
                              key={field}
                              className="flex items-center gap-1"
                            >
                              <span className="text-[6px] text-[#6666cc] uppercase w-12">
                                {field}:
                              </span>
                              <div className="flex-1 border-b border-[#4444aa]/40 px-1 py-0.5 min-h-5">
                                <span
                                  className={`text-[11px] font-bold ${dataInput[field] ? "text-[#aabbff]" : "text-[#444488]"}`}
                                  style={{
                                    textShadow: "0 0 4px rgba(100,100,200,0.3)",
                                  }}
                                >
                                  {dataInput[field] ||
                                    (currentField === field
                                ...
                                {currentField === field && (
                                  <span className="text-[11px] text-[#8888ff] animate-pulse">
                                    █
                                  </span>
                                )}
                              </div>
                        ...
                        {dataTimer !== null && (
                          <div className="mt-1 flex items-center gap-1">
                            <span
                              className={`text-[11px] font-bold ${countdownWarning ? "text-red-400" : "text-[#88aaff]"}`}
                            >
                    ...
                    {(phase === "LOCKING" || phase === "READY_CUT") && (
                      <div className="flex-1 flex flex-col items-center justify-center gap-1">
                        {phase === "LOCKING" && (
                          <>
                            <div
                              className="text-[14px] text-[#88aaff]"
                              style={{
                                textShadow: "0 0 8px rgba(100,100,255,0.5)",
                              }}
                            >
                              ⚙ CHUCK LOCKING...
                            </div>
                            <div className="text-[6px] text-[#556688]">
                              HOLDING LENS SECURE
                            </div>
                            {/* Progress animation */}
                        ...
                        {phase === "READY_CUT" && (
                          <>
                            <div
                              className="text-[14px] text-[#88ff88]"
                              style={{
                                textShadow: "0 0 8px rgba(100,255,100,0.5)",
                              }}
                            >
                              ✓ LENS LOCKED
                            </div>
                            <div className="text-[7px] text-[#668866]">
                              {workTicket.sphere} / {workTicket.cylinder} /{" "}
                              {workTicket.axis}
                            </div>
                    ...
                    {phase === "CUTTING" && (
                      <div className="flex-1 flex flex-col gap-1">
                        ...
                        <div className="mt-0.5">
                          <div className="text-[5px] text-[#6666aa] mb-0.5 uppercase">
                            Tool Pressure / Sag Depth
                          </div>
                          <div className="relative h-6 bg-[#0a0a1a] border border-[#333366] rounded overflow-hidden">
                            ...
                            <div
                              ...
                              style={{
                                ...
                                background:
                                  "linear-gradient(180deg, rgba(40, 255, 40, 0.5), rgba(20, 200, 20, 0.7))",
                    ...
                    {phase === "RESULT" && (
                      <div className="flex-1 flex flex-col items-center justify-center gap-0.5">
                        <div
                          className={`text-[16px] font-bold ${score >= 95 ? "text-[#88ff88]" : score >= 70 ? "text-[#ffcc44]" : "text-[#ff6644]"}`}
                          style={{
                            textShadow:
                              score >= 95
                                ? "0 0 12px rgba(100,255,100,0.6)"
                                : "0 0 8px rgba(200,200,50,0.4)",
                          }}
                        >
                          {score >= 95 ? "★" : score >= 70 ? "✦" : "⚠"} {score}%
                        </div>
                        <div
                          className={`text-[7px] font-bold text-center px-2 ${score >= 95 ? "text-[#88ff88]" : score >= 70 ? "text-[#ffcc44]" : "text-[#ff6644]"}`}
                        >
                          {resultMessage}
                        </div>
                        ...
                        <div className="text-[5px] text-[#445566] mt-1 border-t border-[#334455]/30 pt-1 w-full text-center">
                          SPH: {dataInput.sphere || "---"} | CYL:{" "}
                          {dataInput.cylinder || "---"} | AX:{" "}
                          {dataInput.axis || "---"}
                        </div>
                      </div>
                    )}
                    ...
                    {phase === "FAILURE" && (
                      <div className="flex-1 flex flex-col items-center justify-center gap-0.5">
                        <div
                          className="text-[18px] font-bold text-red-400"
                          style={{ textShadow: "0 0 15px rgba(255,50,50,0.6)" }}
                        >
                          ⚠ CRITICAL
                        </div>
              ...
              <div className="bg-[#e8e0d0] border-4 border-[#4a5252] rounded-lg p-2 shadow-inner">
                <div className="text-[5px] text-[#887766] uppercase font-bold tracking-widest text-center mb-1">
                  DATA KEYPAD
                </div>
            ...
            <div className="flex flex-col gap-2" style={{ width: 230 }}>
              ...
              >
                ...
                <div className="absolute top-1 left-1/2 -translate-x-1/2 z-20 bg-[#3a4242]/80 px-2 py-0.5 rounded">
                  <span className="text-[4px] text-[#889898] font-bold tracking-widest uppercase">
                    CUTTING CHAMBER
                  </span>
                </div>
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, rgba(20,30,40,0.85) 0%, rgba(10,15,20,0.9) 100%)",
                    boxShadow:
                      "inset 0 0 40px rgba(0,0,0,0.8), 0 0 20px rgba(0,0,0,0.5)",
                ...
                >
                  ...
                  {(chuckActive ||
                    ...
                    <>
                      ...
                      {/* Bottom clamp */}
                      <div
                        className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-8 h-3 bg-[#556688] rounded-sm border border-[#7788aa] z-5"
                        style={{ boxShadow: "0 -2px 4px rgba(0,0,0,0.5)" }}
                      />
                    </>
                  )}
                ...
                {spindleRevs && (
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(ellipse at center, rgba(255,100,50,0.08) 0%, transparent 60%)",
                ...
                {cuttingActive && (
                  <div
                    className="absolute bottom-2 left-1/2 -translate-x-1/2 z-7 bg-red-800/60 px-2 py-0.5 rounded animate-pulse"
                    style={{ animationDuration: "0.2s" }}
                  >
                    <span className="text-[5px] text-red-200 font-bold tracking-wider">
                      CUTTING
                    </span>
                  </div>
                )}
                ...
                <div className="absolute bottom-1 right-1 z-8">
                  <span className="text-[3px] text-[#444455]">
              ...
              >
                ...
                <div className="grid grid-cols-3 gap-0.5 text-center">
                  <div>
                    <div className="text-[5px] text-[#aa8866] uppercase">
                      Sphere
                    </div>
                    <div className="text-[14px] font-bold text-[#443322]">
                      {workTicket.sphere}
                    </div>
                  </div>
                  <div>
                ...
                {phase === "RESULT" && (
                  <div
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[16px] font-bold text-red-400/30 rotate-[-20deg] select-none"
          ...
          <div className="mx-3 mb-3 p-2 bg-[#6a7474] border-2 border-[#4a5252] rounded-lg flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                ...
                <RollerCounter
                  value={phase === "CUTTING" ? Math.round(timingPos) : score}
                  label={phase === "CUTTING" ? "POS" : "SCORE"}
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <motion.button
                ...
                style={{
                  boxShadow:
                    phase === "READY_CUT"
                      ? "0 0 12px rgba(50,200,50,0.3), 0 2px 4px rgba(0,0,0,0.3)"
                      : "0 2px 4px rgba(0,0,0,0.3)",
                }}
              >
                <div className="flex flex-col items-center leading-none">
