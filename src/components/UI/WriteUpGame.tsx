...
export const WriteUpGame: React.FC<WriteUpGameProps> = ({
  ...
  const [services, setServices] = useState<ServiceItem[]>([
    {
      ...
      name: "Eye Exam",
      price:
        insurance && insurance !== "none" && insurance !== "MEDICAID" ? 50 : 75,
      checked: false,
    },
    {
      id: "glasses",
      name: "Complete Pair Glasses",
      price: 150,
      checked: false,
    },
  ...
  const handleComplete = () => {
    ...
    if (missing.length === 0 && extra.length === 0) {
      ...
      setFeedback(
        "That doesn't look right. Make sure you select exactly what I asked for!",
      );
    }
  };
    ...
    >
        ...
        <div className="mt-12 text-center bg-black/40 p-6 rounded-xl border-4 border-black/20">
          ...
          {prescription && (
            <div className="mt-6 bg-white/10 border-2 border-white/20 p-4 rounded text-left">
              <h3 className="text-white/50 font-black uppercase tracking-widest mb-2 text-sm">
                PRESCRIPTION
              </h3>
              <div className="font-mono text-white text-lg grid grid-cols-3 gap-8">
                <div>
                  <span className="text-white/50 text-sm">SPH:</span> <br />
                  {prescription.sphere}
                </div>
                <div>
                  <span className="text-white/50 text-sm">CYL:</span> <br />
                  {prescription.cylinder}
                </div>
                <div>
                  <span className="text-white/50 text-sm">AXS:</span> <br />
                  {prescription.axis}°
                </div>
              </div>
            </div>
      ...
      <div className="w-1/2 h-full flex flex-col p-8 bg-white overflow-y-auto">
          ...
          {services.map((service) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`w-8 h-8 rounded-lg border-4 flex items-center justify-center transition-colors ${
                    service.checked
                ...
                <span
                  className={`font-bold text-lg ${service.checked ? "text-blue-900" : "text-slate-700"}`}
                >
                  {service.name}
                </span>
              </div>
              <span
                className={`font-black text-xl ${service.checked ? "text-blue-600" : "text-slate-400"}`}
              >
                ${service.price}
              </span>
            </motion.div>
        ...
        <div className="border-t-4 border-slate-200 pt-6 mt-6 bg-slate-50 -mx-8 -mb-8 p-8 rounded-t-3xl">
          ...
          <div className="flex justify-between items-center mb-6">
            <span className="text-2xl font-black uppercase text-slate-500">
              TOTAL DUE
            </span>
