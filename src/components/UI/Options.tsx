    ...
    >
      ...
      <div className="bg-black border-4 border-white p-8 md:p-12 shadow-[12px_12px_0_0_rgba(59,130,246,1)] w-full max-w-md pixel-border">
        <div className="flex items-center gap-6 mb-12 border-b-4 border-white pb-6">
          ...
          <h2 className="text-2xl font-black text-white italic tracking-tighter">OPTIONS_MGMT</h2>
        </div>

        <div className="space-y-10">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-[8px] font-black text-blue-400 uppercase tracking-widest">MASTER_VOL</label>
              <span className="text-white text-[8px] font-black">{Math.round(settings.volume * 100)}%</span>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-[8px] font-black text-blue-400 uppercase tracking-widest">SFX_CHANNEL_01</label>
              <span className="text-white text-[8px] font-black">85%</span>
            </div>
            <div className="relative h-6 flex items-center">
              <div className="absolute inset-0 bg-white/20 border-2 border-white/10" />
              <div className="absolute inset-y-0 left-0 bg-yellow-500 shadow-[2px_0_0_0_white]" style={{ width: '85%' }} />
            </div>
          </div>

          <div className="flex items-center justify-between p-4 border-4 border-white/20 hover:border-white transition-all">
            <div className="flex items-center gap-3">
              <Volume2 className={`w-4 h-4 ${settings.muted ? 'text-red-500' : 'text-blue-500'}`} />
              <span className="font-black text-white uppercase text-[8px] tracking-widest">MUTE_BG_MUSIC</span>
            </div>
            <button
              onClick={() => onUpdate({ ...settings, muted: !settings.muted })}
              className={`w-12 h-6 border-2 border-white transition-all p-1 ${settings.muted ? 'bg-red-600' : 'bg-green-600'}`}
            >
              <div className={`h-full w-1/2 bg-white ${settings.muted ? 'translate-x-full' : ''} transition-transform`} />
            </button>
          </div>
        </div>
